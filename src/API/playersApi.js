export const fetchPlayer = (name) => {
    return fetch(`${process.env.REACT_APP_BACKEND}/players/${name}`)
        .then(response => {
...
export const fetchPlayerNames = (hideInactivePlayers) => {
    return fetch(`${process.env.REACT_APP_BACKEND}/players/player-names?active=${hideInactivePlayers}`)
        .then(response => response.json())
}

export const fetchPlayerSigns = (player, tradeType, page) => {
    return fetch(`${process.env.REACT_APP_BACKEND}/players/${player.name}/chest-shops?tradeType=${tradeType}&page=${page}`)
        .then(response => response.json())
}

export const fetchRegionsByPlayer = (name, page) => {
    return fetch(`${process.env.REACT_APP_BACKEND}/players/${name}/towns?page=${page}`)
        .then(response => response.json())
}
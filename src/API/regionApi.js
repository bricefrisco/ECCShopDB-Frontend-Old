export const fetchRegionNames = (server, hideDisabledRegions) => {
    return fetch(`${process.env.REACT_APP_BACKEND}/regions/region-names?server=${server}&active=${hideDisabledRegions}`)
        .then(response => response.json())
}

export const fetchRegion = (server, name) => {
    return fetch(`${process.env.REACT_APP_BACKEND}/regions/${server}/${name}`)
        .then(response => {
...
export const fetchRegionSigns = (region, tradeType, page) => {
    return fetch(`${process.env.REACT_APP_BACKEND}/regions/${region.server}/${region.name}/chest-shops?page=${page}&tradeType=${tradeType}`)
        .then(response => response.json())
}

export const getPlayersByRegion = (server, name, page) => {
    return fetch(`${process.env.REACT_APP_BACKEND}/regions/${server}/${name}/mayors?page=${page}`)
        .then(response => response.json())
}
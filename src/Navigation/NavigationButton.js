...
const NavigationButton = ({text, selected}) => {
    ...
    if (text === 'forum') {
        return (
            <a href='https://ecocitycraft.com/forum/threads/shopdb-applications-suggestions-bug-reports-feedback-faq.205318/'
               rel='noopener noreferrer' target='_blank' className={classes.link}>
    ...
    return (
        <Link to={getNavigationUri(text)} className={classes.link}>
            <div className={classes.navigationButton}>
                <Typography
                    className={selected ? classes.textSelected : classes.text}>{getButtonText(text)}</Typography>
            </div>
        </Link>
    )
}

...
const ChestShops = () => {
    ...
    useEffect(() => {
        ...
        setLoading(true)
        fetchChestShopSigns(page, server, tradeType, sortBy, hideIdenticalSigns, hideOutOfStockSigns, material)
            .then(({currentPage, totalElements, results}) => {
                setPagination({currentPage, totalElements})
                setSigns(results)
                setLoading(false)
            })
    }, [page, server, tradeType, sortBy, hideIdenticalSigns, hideOutOfStockSigns, material])

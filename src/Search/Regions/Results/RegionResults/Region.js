...
const Region = ({region}) => {
    ...
    const getMayors = (mayorNames) => {
                ...
                if (idx === 3) {
                    return (
                        <div className={classes.mayors} key={idx}>
                            <span onClick={(event) => navigateToPlayer(event, mayorName)} className={classes.link}>
                                {mayorName}
                            </span>... ({mayorNames.length - 3} more)
                        </div>)
                } else {
                    return (
                        <div className={classes.mayors} key={idx}>
        ...
        return mayorNames.map((mayorName, idx) => {
            if (idx === mayorNames.length - 1) {
                return (
                    <span onClick={(event) => navigateToPlayer(event, mayorName)} key={idx} className={classes.link}>
                        {mayorName}
                    </span>)
            } else {
                return (
    ...
    return (
        <div className={classes.card} onClick={(event) => navigateToRegion(event, region.server, region.name)}>
            <div className={classes.cardContents}>
                <StoreIcon className={getRandomBackgroundColor(region.name)}/>
                <div style={{width: '100%'}}>
                    <div style={{display: 'flex', justifyContent: 'space-between'}}>
                        <span className={classes.headline}>{region.name.toLowerCase()}</span>
                        <span className={classes.updatedTime}>Updated {getTimeUpdatedAgo(region['lastUpdated'])}</span>
                    </div>
                    <span className={classes.additionalInfo}>Mayors: {getMayors(region['mayors'])}</span>
                    <span className={classes.description}>
                        {region['name'].toLowerCase()} is a {region['active'] ? 'shop ' : 'town '}
                        on {region.server} with {region['mayors'].length}{' '}
                        {region['mayors'].length === 1 ? 'mayor' : 'mayors'} and {region['numChestShops']} chest shops.
                    </span>
